...
const PopularBrands = () => {
  ...
  const dataToRender = [...visibleBrands, {id: 'moreOrLess'}];
  ...
  const fetchBrands = async () => {
    try {
      ...
      if (
        ...
        console.log(
          `Fetched ${sortedBrands.length} brands with ${
            sortedBrands.filter(b => b.logo).length
          } logos`,
        );
    ...
    if (item.id === 'moreOrLess') {
      return (
        <TouchableOpacity
          ...
          <Text style={[styles.brandName, {color: COLORS1?.textDark}]}>
      ...
      <TouchableOpacity
        ...
        <View style={styles.logoContainer}>
          {item.logo ? (
            ...
            <Text style={[styles.brandLogo, {color: COLORS1?.textDark}]}>
              {placeholder.text}
            </Text>
          ) : (
            <Text style={[styles.brandInitial, {color: COLORS1?.textDark}]}>
              {item.name[0]}
            </Text>
          )}
        </View>
        <Text
          style={[styles.brandName, {color: COLORS1?.textDark}]}
          numberOfLines={1}>
  ...
  const renderSeeAllItem = () => {
    // <TouchableOpacity style={styles.brandItem} onPress={navigateToAllBrands}>
    //   <View style={styles.logoContainer}>
    //     <Text style={styles.ellipsis}>•••</Text>
    //   </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          keyExtractor={(item, index) =>
            item.id === 'moreOrLess' ? 'moreOrLess' : item.id.toString()
          }
